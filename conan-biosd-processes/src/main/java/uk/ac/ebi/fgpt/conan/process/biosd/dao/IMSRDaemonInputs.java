...
public class IMSRDaemonInputs implements ConanDaemonInputsDAO {

	private Logger log = LoggerFactory.getLogger(getClass());

	public Class<? extends ConanParameter> getParameterType() {
		return SampleTabIMSRAccessionParameter.class;
	}
	...
	public List<String> getParameterValues() {
		...
		File dir = new File(sampletabpath, "imsr");
		for (File subdir : dir.listFiles()) {
			if (subdir.isDirectory()) {
				String staccession = subdir.getName();				
				File tab = new File(subdir, "raw.tab.txt");
				File sampletab = new File(subdir, "sampletab.pre.txt");
				
				if (tab.exists()){
					if (!sampletab.exists() 
							|| tab.lastModified() > sampletab.lastModified()  ){
